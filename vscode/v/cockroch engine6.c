 ...
 if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
 {
    ...
    else if(gameState == -1) // Map Editor
    {
        ...
        if(x > 850 && x < 950)
        {
            ...
            if(y > 70 && y < 120){ buttonState = 10; } // Enemy
            if(y > 130 && y < 180){ buttonState = 4; } // Key
        ...
        if(buttonState == 3) // Player
        {
            pa+=45; if(pa>359){ pa-=360;} pdx=cos(degToRad(pa)); pdy=-sin(degToRad(pa));
            buttonState = 0;
        }
        if(buttonState == 4) // Add Enemy
        {
            int s=numSprite;
            sp[s].type=3; sp[s].state=1; sp[s].map=2; sp[s].x=px; sp[s].y=py; sp[s].z=20;
            sp[s].r=255; sp[s].g=0; sp[s].b=0;
            numSprite++;
            buttonState = 0;
        }
...
void drawPaletteTile(int v, int posX, int posY, int size)
{
    int x,y;
    int colors[8][3] = {
...
void mapEditor()
{
    ...
    for(i=0; i<tileCount; i++)
    {
        drawPaletteTile(tiles[i], 10 + i*70, 580, 16);
...
void init()//init all variables when game starts
{
 ...
 sp[0].r=255; sp[0].g=255; sp[0].b=0; //key (YELLOW)
 sp[1].r=255; sp[1].g=  0; sp[1].b=0; //enemy (RED)
